...
def f(df, text_col_name="text", labels_col_name="topic"):
    ...
    document_lengths_histogram = np.histogram(
        s_document_lengths.values,
        bins=20
    )
    ...
    number_of_duplicates = description.loc["number of documents"].values[0][0] - \
    ...
    schart = go.Sankey(
        node=dict(
            pad=15,
            thickness=20,
            label=["Total Number of Documents", "Missing Documents", "Unique Documents", "Duplicate Documents"],
        ...
        link=dict(
            ...
            value=[description.loc["number of unique documents"].values[0][0], description.loc["number of missing documents"].values[0][0], number_of_duplicates]
        ))

    # Create Table to show the 10 most common words (with and without stopwords)
    table = go.Table(header=dict(values=['Top Words with Stopwords', 'Top Words without Stopwords']),
                      cells=dict(values=[description.loc["most common words"].values[0][0], description.loc["most common words excluding stopwords"].values[0][0]]))
            

    # Combine figures.
    fig.add_trace(
        label_distribution_pie_chart_fig,
        row=2, col=2
    )
    fig.add_trace(
        document_lengths_fig,
        row=2, col=1
    )
